...
if not (os.path.exists(train_path) and os.path.exists(test_path)):
    raise FileNotFoundError("Please upload train.csv and test.csv then re-run.")
...
def norm_month(x):
    ...
    s = str(x).strip()
    if s.lower() in month_map: return month_map[s.lower()]
    cap = s[:3].title()
...
train['spend'] = pd.to_numeric(train['spend'], errors='coerce').fillna(0).round(2)
...
compare_result.to_csv('predictions.csv', index=False)
